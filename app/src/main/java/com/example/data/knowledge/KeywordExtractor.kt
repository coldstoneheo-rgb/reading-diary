 ...
 * - `다`로 끝나면 활용형으로 본다. 그래서 `인격체다`도 버려진다(명사 예외는 [PROTECTED_NOUNS]).
...
object KeywordExtractor {
    ...
    /** 1글자 용언 어미. `위대한`→`위대`, `생각하`→`생각`. 2글자 어간을 남길 수 있을 때만 뗀다(`제한`·`인하`는 보존). */
    ...
    private fun isProtectedNoun(word: String): Boolean =
        word in PROTECTED_NOUNS || word.endsWith("주의")
