...
원칙은 [`../../../CLAUDE.md`](../../../CLAUDE.md), 메커니즘은 [`../../HARNESS.md`](../../HARNESS.md).
핵심: **Maker와 Critic을 분리**하고, **승인 강도를 위험에 비례**시킨다.

## 작업 루프 (0→10)

```
0.  컨텍스트 로드 — MEMORY.md 확인, codebase-memory list_projects → 없으면 index_repository
1.  현 상태 동기화 — git status/원격/worktree 잔존 확인. main은 pull --ff-only
2.  다음 작업 선정 — 리스크 최소화 우선. 자율영역 ↔ 인간필요영역(서명·스키마)으로 분해
3.  계획 + 완료정의(DoD) — 범위/비범위/변경파일/검증 명령
4.  위험등급 게이트 — HARNESS.md 표. 🔴이면 명시 승인 후 진행
5.  worktree 브랜치 — EnterWorktree(name). 오래된 브랜치 재사용 금지
6.  구현(Maker) — 승인 계획 내에서만. 범위·위험 커지면 멈추고 재보고
7.  Critic 게이트 — critic 서브에이전트 채점 → 7/10 미달·심각 결함 시 보정 루프
8.  검증 — `gradle :app:testDebugUnitTest` 등 변경 부위 타깃 테스트. 결과 PASS/MINOR/BLOCKER + "왜 안전한가"
9.  commit → push → PR(본문: Summary/Verification/Notes) → 리뷰 대응(봇 맹신 금지) → squash 머지
...
- Roborazzi 스크린샷 테스트는 기준 이미지(`app/src/test/screenshots/`)가 커밋됨. 의도된 UI 변경만 `recordRoborazziDebug`로 갱신하고 PR에 이유를 적는다.
- `GeminiApiClient`는 오프라인 스텁. AI 기능 요구는 백엔드·키 정책 합의가 먼저다.
- 릴리스 서명은 사용자 전용(`!`).
